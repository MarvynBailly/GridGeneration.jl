...
initialGrid = GetAirfoilGrid(airfoilPath = "examples/airfoil/A-airfoil.txt", radius = 3)
...
for (problem, name) in zip(problems, names)
    ...
    p6 = scatter(xs, m, title = "M values", label="M(s)", color=:black, markersize=2, markerstrokewidth=0)
    display(p6)
end