...
tree, refs = setup_metric_tree(metricData, PtRef[])


scale = 0.001
M_func = (x,y) -> scale * find_nearest_kd(metricData, tree, refs, x, y)
m = GridGeneration.Get1DMetric(boundarySection, M_func)
...
diff_opt = diff(x_sol_opt)
diff_opt = [0, diff_opt...]


p1 = plot(xs, m, title = "1D Metric with Boundary Spacing",
...
p2 = plot(title = "Optimal and Non-Optimal 1D Distributions",
        xlabel = "s", ylabel = "m(x(s))", label = "m(x(s))",
        legend = :best, linewidth=2)
...
plot!(p2, sol_opt[1,:], diff_opt, label="Non-Optimal Solution Difference", color=:red, linewidth=1)
...
if saveFig
    savefig(p4, path * "ode_solution_real_metric_sparser.svg")
else
    display(p4)
end